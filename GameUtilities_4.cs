...
public static class GameUtilities_4
{
    ...
    public static IEnumerator AnimatePotentialMatches(IEnumerable<GameObject> potentialMatches)
    {
        for (float i = 1f; i >= 0.3f; i -= 0.1f)
        {
            ...
            yield return new WaitForSeconds(ConstantsVariable_4.OpacityAnimationFrameDelay);
        }
    ...
    public static List<GameObject> CheckHorizontal1(int row, int column, ShapesArray_4 shapes)
    {
        if (column <= ConstantsVariable_4.Columns - 2)
        {
                ...
                IsSameType(shapes[row, column + 1].GetComponent<Shape_4>()))
            {
                if (row >= 1 && column >= 1)
                if (shapes[row, column].GetComponent<Shape_4>().
                    IsSameType(shapes[row - 1, column - 1].GetComponent<Shape_4>()))
                ...
                if (row <= ConstantsVariable_4.Rows - 2 && column >= 1)
                if (shapes[row, column].GetComponent<Shape_4>().
                    IsSameType(shapes[row + 1, column - 1].GetComponent<Shape_4>()))
                        return new List<GameObject>()
                                {
    ...
    public static List<GameObject> CheckHorizontal2(int row, int column, ShapesArray_4 shapes)
    {
        if (column <= ConstantsVariable_4.Columns - 3)
        {
            if (shapes[row, column].GetComponent<Shape_4>().
                IsSameType(shapes[row, column + 1].GetComponent<Shape_4>()))
            {

                if (row >= 1 && column <= ConstantsVariable_4.Columns - 3)
                if (shapes[row, column].GetComponent<Shape_4>().
                    IsSameType(shapes[row - 1, column + 2].GetComponent<Shape_4>()))
                        return new List<GameObject>()
                                {
                                    shapes[row, column],
                                    shapes[row, column + 1],
                ...
                if (row <= ConstantsVariable_4.Rows - 2 && column <= ConstantsVariable_4.Columns - 3)
                if (shapes[row, column].GetComponent<Shape_4>().
                    IsSameType(shapes[row + 1, column + 2].GetComponent<Shape_4>()))
                        return new List<GameObject>()
                                {
    ...
    public static List<GameObject> CheckVertical1(int row, int column, ShapesArray_4 shapes)
    {
        if (row <= ConstantsVariable_4.Rows - 2)
        {
            if (shapes[row, column].GetComponent<Shape_4>().
                IsSameType(shapes[row + 1, column].GetComponent<Shape_4>()))
            {
                ...
                if (column <= ConstantsVariable_4.Columns - 2 && row >= 1)
                if (shapes[row, column].GetComponent<Shape_4>().
                    IsSameType(shapes[row - 1, column + 1].GetComponent<Shape_4>()))
                        return new List<GameObject>()
                                {
